...
game:GetService("StarterGui"):SetCore("SendNotification",
    {
        Title = "Онлайн переводчик",
        Text = "Подключен к АПИ Гугла",
        Duration = 3
    }
)
                  
properties.Text = "[ПЕРЕВОДЧИК] Синтаксис > (пример >ru ; >en), откючить переводчик - >d."
...
function disableSend()
    sendEnabled = false
    properties.Text = "[TR] Перевод отключен"
    StarterGui:SetCore("ChatMakeSystemMessage", properties)
end
                ...
                if Enter ~= false and Bar.Text ~= '' then
                    ...
                    elseif Message:sub(1,1) == ">" and not Message:find(" ") then
                        if getISOCode(Message:sub(2)) then
                            ...
                            properties.Text = "[ПЕРЕВОДЧИК] Алё, такого языка нету!!!!"
                            StarterGui:SetCore("ChatMakeSystemMessage", properties)
                        end
